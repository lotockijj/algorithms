...
public class BST_3_5_9 <Key extends Comparable<Key>, Value> {
    private Node root; // root of BST

    private class Node {
        private Key key; // key
        private List<Value> val; // associated value
    ...
    private List<Value> get(Node x, Key key) { // Return value associated with key in the subtree rooted at x;
    ...
    private Node put(Node x, Key key, Value val) {
        ...
        if (x == null) return new Node(key, Arrays.asList(val).stream().collect(Collectors.toList()), 1);
        int cmp = key.compareTo(x.key);
        if (cmp < 0) x.left = put(x.left, key, val);
        else if (cmp > 0) x.right = put(x.right, key, val);
        else {
            List<Value> values = x.val;
            values.add(val);
        }
